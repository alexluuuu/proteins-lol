...
class ImageDataGenerator:
    ...
    def next_batch(self, batch_size):
        ...
        for i in range(len(paths)):
            #img = cv2.imread(paths[i]) #change for dcm images
            ds = pydicom.read_file(paths[i])
            img = ds.pixel_array
            img = ds.RescaleSlope * img + ds.RescaleIntercept #rescale to hounsfield units
            img = np.stack((img,)*3, -1) # Make it 3 channel
